...
class Paginator {
	...
	/**
     * @var int calculate displayed page
     */
     ...
     * @param int $midRange
     */
    function __construct($itemsCount, $currentPage = 1, $limit = 20, $midRange = 5)
    {
    ...
    {
        /*$start = $this->currentPage/$this->limit - floor($this->midRange/2);
		$end = $this->currentPage/$this->limit+ floor($this->midRange/2)+1;
		
		if($start < 0 and $this->currentPage/$this->limit < 1)
		{
			$start = 1;
			$end += abs($start);
		}
		else if($start<0 and $this->currentPage/$this->limit == 1)
		{
			$start = 2;
			$end += abs($start)-1;
		}
		...
		{
			$start -= $end-$this->numPages;
            $end = $this->numPages;
		}
        
        $this->range = range($start, $end);*/
        $startRange = $this->currentPage/$this->limit - floor($this->midRange/2-1);
        $endRange = $this->currentPage/$this->limit + floor($this->midRange/2+1);
