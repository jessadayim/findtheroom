...
class RegisterController extends Controller
{
    ...
    public function alertEmailAction()
    {
        return $this->render('FTRWebBundle:Register:alertemail.html.twig');
    }
                ...
                try{
                    ...
                    if(count($objSQL1)>0){
                        ...
                        try{
                            ...
                            $sqlOwner = "SELECT email,confirm_token,firstname,lastname
                                         FROM user_owner
                                         WHERE confirm_token = '$random_token' AND deleted = 0";
                            $objOwner = $conn -> fetchAll($sqlOwner);
                            $token = $objOwner[0]['confirm_token'];
                            $email = $objOwner[0]['email'];
...
ติดต่อสอบถามข้อมูลเพิ่มเติม โทร 02-692-1199";
                            ...
                            return $this->render('FTRWebBundle:Publish:publish.html.twig',array('enable'=>true));
