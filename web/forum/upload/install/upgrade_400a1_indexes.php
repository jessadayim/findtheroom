...
php upgrade_400a1_indexes.php

If you don't have access to the command line you may run it via the web as
http://{path_to_vb}/install/upgrade_400a1_indexes.php
...
define('THIS_SCRIPT', 'upgrade_400a1_indexes.php');
...
if (!empty($post_index_query))
{
	echo $upgrade_phrases['upgrade_400a1_indexes.php']['adding_post_indexes'];
	echo "\n";
	if (run_index_query($vbulletin->db, $post_index_query, $upgrade_phrases))
	{
		echo $upgrade_phrases['upgrade_400a1_indexes.php']['post_index_success'];
		echo "\n";
	}
}
...
if (!empty($thread_index_query))
{
	echo $upgrade_phrases['upgrade_400a1_indexes.php']['adding_thread_indexes'];
	echo "\n";
	if (run_index_query($vbulletin->db, $thread_index_query, $upgrade_phrases))
	{
		echo $upgrade_phrases['upgrade_400a1_indexes.php']['thread_index_success'];
		echo "\n";
	}
	$db->query_write(sprintf($thread_index_query, TABLE_PREFIX));
}
