...
class vBForum_Search_IndexController_Forum extends vB_Search_IndexController
{
  ...
  public function __construct()
  {
	...
	public function index($id)
	{
		...
		// make sure forum is not the cms comment forum
		if ($row AND $row['forumid'] != $vbulletin->options['vbcmsforumid'])
		{
	...
	public function index_id_range($start, $finish)
	{
		...
		{
			// do not index the cms comment forum
			if ($row['forumid'] == $vbulletin->options['vbcmsforumid'])
			{
				continue;
			}
			$fields = $this->record_to_indexfields($row);
